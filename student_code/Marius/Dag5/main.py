...
class Background(BaseWidget):
    ...
    tx_asphalt = ObjectProperty(None)

    def __init__(self, **kwargs):
        super(Background, self).__init__(**kwargs)
        self.load_tileable('asphalt')
...
class KivyBirdApp(App):
    ...
    def on_start(self):
        self.state = "wait"
        self.spacing = 0.3 * self.root.width #0.5
    ...
    def update(self, nap):
        if self.state == "wait":
            self.bird.source ="images/Tesla_ferdigklippet.png"
            self.bird.size = (110, 110)
            ...
            if self.test_game_over():
                self.bird.source ="images/eksplosjon.png"
                self.bird.size = (10, 10)
                self.playing = False
                self.teller = 0
    ...
    def test_game_over(self):
        ...
        if self.bird.y < 1 or self.bird.y > screen_height - 50:
            return True
