...
class Background(BaseWidget):
    ...
    tx_background2 = ObjectProperty(None)
    ...
    def update(self, nap):
        ...
        if App.get_running_app().root.ids.score_label.text >= "2":
            if not self.bg2:
                self.bg2 = True
                self.load_tileable("background2")
                self.set_background_size(self.tx_background2)
                self.canvas.clear()
                with self.canvas:
                    self.rect = Rectangle(pos=self.pos, 
                        size=(self.width, self.height),
                        texture=self.tx_background2)
            self.set_background_uv('tx_background2', 2 * nap)
...
class Pipe(BaseWidget):
    ...
    tx_sandstone_1 = ObjectProperty(None)
    tx_sandstone_down = ObjectProperty(None)
    ...
    def __init__(self, **kwargs):
        ...
        for name in ('sandstone_1', 'sandstone_down'):
